...
const CraftMitraModal = ({ isOpen, onClose }) => {
  ...
  const [promptText, setPromptText] = useState('Click to start speaking');
        ...
        mediaRecorderRef.current.onstop = () => {
          ...
          reader.onloadend = async () => {
            try {
              ...
              await currentUser.getIdToken(true);
              const result = await getCraftMitraResponse({ audioData: base64data });
              ...
              if (error.code === 'functions/unauthenticated' || error.message.includes("logged in")) {
                setStatusText("Authentication failed. Please log in again.");
              } else {
