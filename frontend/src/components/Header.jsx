...
const Header = ({ onSignInClick, onNavigate, onNavigateAndScroll }) => { 
  ...
  useEffect(() => {
    if (currentUser) {
      const getUserProfile = async () => { const userDocRef = doc(db, 'users', currentUser.uid);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => { if (!event.target.closest('.profile-dropdown-container')) {
  ...
  useEffect(() => {
    const translateContent = async () => {
      ...
      try {
        ...
        setContent({
          discover: translations[0], regions: translations[1], artisans: translations[2],
          stories: translations[3], giftingAssistant: translations[4], myDashboard: translations[5],
          shop: translations[6], editProfile: translations[7], signOut: translations[8],
  ...
  const handleDropdownNavigate = (page) => { onNavigate(page);
     setIsProfileDropdownOpen(false);
  };
  const handleLanguageChange = (langCode) => { changeLanguage(langCode);
     setIsLangDropdownOpen(false);
  };
          ...
          <div className="profile-dropdown-container">
            ...
            {isProfileDropdownOpen && (
              <div className="profile-dropdown-menu">
                ...
                <button onClick={logout} className="nav-link dropdown-signout">{content.signOut}</button>
              </div>
            )}
          </div>
        ) : (
