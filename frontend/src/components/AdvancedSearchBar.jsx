...
const englishContent = {
  ...
  alertMessage: "Sorry, we couldn't find products matching that image.",
};
...
const AdvancedSearchBar = ({ onSearch }) => {
  ...
  useEffect(() => {
    if (recognition) {
      const langCode = currentLanguage.code === 'en' ? 'en-IN' : currentLanguage.code;
  ...
  useEffect(() => {
    const translateContent = async () => {
      ...
      try {
        ...
        setContent({
          searching: translations[0],
          listening: translations[1],
          placeholder: translations[2],
          alertMessage: translations[3],
        });
      } catch (err) {
        console.error("Failed to translate AdvancedSearchBar content:", err);
        setContent(englishContent);
      } finally {
    ...
    <div className={`advanced-search-container ${isTranslating ? 'translating' : ''}`}>
      ...
      <form onSubmit={handleTextSearch} className={`search-bar ${status !== 'idle' ? 'disabled' : ''}`}>
        ...
        <input
          ...
          placeholder={status === 'listening' ? content.listening : content.placeholder}
